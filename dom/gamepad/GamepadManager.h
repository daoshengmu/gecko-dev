                             ...
                             public nsIIPCBackgroundChildCreateCallback
{
 public:
  ...
  void AddGamepad(uint32_t aIndex, const nsAString& aID, GamepadMappingType aMapping,
                  GamepadChannelType aChannel, uint32_t aNumButtons, uint32_t aNumAxes);

  // Remove the gamepad at |aIndex| from the list of known gamepads.
  void RemoveGamepad(uint32_t aIndex, GamepadChannelType aChannel);

  // Update the state of |aButton| for the gamepad at |aIndex| for all
  // windows that are listening and visible, and fire one of
  // a gamepadbutton{up,down} event at them as well.
  // aPressed is used for digital buttons, aValue is for analog buttons.
  void NewButtonEvent(uint32_t aIndex, GamepadChannelType aChannel, uint32_t aButton,
 ...
 private:
  ...
  // is from VRManager.
  uint32_t GetGamepadIndexWithChannel(uint32_t aIndex, GamepadChannelType aChannel);
